...
stats = lapply(1:length(folds), function(fold) {
  ...
  iterator = unique(sort(pb_occ_tab[pb_occ_tab>=n_boot/6]))
...
plot(sort(unique(stats$occurence)), sapply(sort(unique(stats$occurence)), function(o) mean(stats[stats$occurence==o,]$test_err)), lwd=3, col=2)
