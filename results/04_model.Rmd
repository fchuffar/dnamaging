...
plot(0, 0, col=0, xlab="log10(lambda)", ylab="RMSE", xlim=log10(range(cvrecaps$lambda)), ylim=range(cvrecaps$rmse))
...
plot(0, 0, col=0, xlab="log10(lambda)", ylab="nbprobes", xlim=log10(range(cvrecaps$lambda)), ylim=range(cvrecaps$nbprobes))
lapply(alphas, function(alpha) { 
  cvrecap = cvrecaps[cvrecaps$alpha==alpha,]
...
# With lambda min 
#model_params = lapply(alphas, function(alpha, cvrecaps=cvrecaps) { 
#  cvrecap = cvrecaps[cvrecaps$alpha==alpha,]
...
models = lapply(model_params, function(m_params, idx_train=idx_train, y_key=y_key) { 
  print(m_params)
...
layout(matrix(1:2,1), respect=TRUE)
layout(matrix(1:12,2), respect=TRUE)
...
if (USE_PARAPPLY) {
  ...
  bs_probes = parallel::parApply(cl, t(t(1:n_boot)), 1, bs_func, idx_train=idx_train, model_params=model_params)
} else {
  print("bootstrap using epimedtools::monitored_apply")
  bs_probes = epimedtools::monitored_apply(mod=1, t(t(1:n_boot)), 1, bs_func, idx_train=idx_train, model_params=model_params)
}
```
...
models_bs = lapply(1:length(alphas), function(i){ # Compute coefficients and mean in the train for each probes of the model for each alpha
