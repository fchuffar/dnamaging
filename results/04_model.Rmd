...
nbprobes.lim = c(min(nbprobes.lim),max(nbprobes.lim))
...
lapply(stat, function(it) { 
  cvrecap = it$cvrecap
  points(log(cvrecap$lambda),cvrecap$RMSE,col = col[as.character(it$alpha)])
  abline(v = log(it$lambdamin), col = col[as.character(it$alpha)])
  })
legend(x="topleft", legend=names(col), fill=col, title="Alphas")

plot(0,0,col = 0, xlab="log(lambda)", ylab="nbprobes", xlim=lambda.lim, ylim=nbprobes.lim, main="nbprobes depending on alphas and lambdas" )
lapply(stat, function(it) { 
  cvrecap = it$cvrecap
  points(log(cvrecap$lambda),cvrecap$nbprobes, col = col[as.character(it$alpha)])
  abline(v = log(it$lambdamin), col = col[as.character(it$alpha)])
  })
legend(x="topleft", legend=names(col), fill=col, title="Alphas")
