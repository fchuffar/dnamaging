...
if (!exists("mcvaglmnet")) {
  cvaglmnet = function(alphas, idx_train, y_key) {
    x = mget_full_cpg_matrix(idx_train)
    y = mget_df()[idx_train, y_key] # service. Design Oatterns, Gamma et al. 
    ...
    modelcva = glmnetUtils::cva.glmnet(x=x, y=y, type.measure="mse", standardize=TRUE, outerParallel = cl)
    # cvrecap = data.frame(lambda=modelcv$lambda, rmse=sqrt(modelcv$cvm), nbprobes=modelcv$nzero, alpha=alpha)
    return(modelcva)  
  }
...
modelcva = mcvaglmnet(alphas=alphas, idx_train=idx_train, y_key=y_key)
length(modelcva)
...
best_model
m_params = c(alpha=best_model[["alpha"]], lambda=best_model[["lambda"]])
```
...
```{r glmnet_build, echo=FALSE, eval=FALSE}
...
bootstrap Cross validation for best lambda and alpha, **FCh: No, it is wrong.**
...
bs_func = function(i, idx_train, model_params) { #epimedtools to see evolution of running 
  ...
  names(models_bs) = paste0(sapply(models, "[[", "name"), "_rnd", i)
...
if (USE_PARAPPLY) {
  ...
  bs = parallel::parApply(cl, t(t(1:n_boot)), 1, bs_func, idx_train=idx_train, model_params=model_params)
} else {
...
bs_probes = unlist(bs, recursive=FALSE)
length(bs_probes)
