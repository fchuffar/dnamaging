...
stats = epimedtools::monitored_apply(mod=1, t(t(iterator)), 1, function(occ) {
  ...
  stats = lapply(1:length(folds), function(fold) {
    ...
    ret = mcall_glmnet_mod(tmp_idx_train, tmp_idx_test, y_key, tmp_probes, occ, fold, sub_df, alpha=best_model_cvaglmnet_params$alpha)
    return(ret)
  })
...
if (!is.null(occ_optim)) {
  idx = which(as.numeric(do.call(rbind,strsplit(bar$names, " ")>=occ_optim)[,1]))
