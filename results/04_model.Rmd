...
if (!exists("mcvglmnet")) {
  cvglmnet = function(alpha, idx_train, y_key) {
    Xtrain = mget_full_cpg_matrix()[idx_train,]
    Ytrain = mget_df()[idx_train, y_key]
    modelcv = glmnet::cv.glmnet(Xtrain, Ytrain, alpha=alpha, type.measure="mse", standardize=TRUE)
...
stat = lapply(alphas, function(alpha, idx_train=idx_train, y_key=y_key) { #list with cv recap (lambda,rmse,nbprobes) and lambda.min  
  print(alpha)
  modelcv = mcvglmnet(alpha=alpha, idx_train=idx_train, y_key=y_key)
  return(cvrecap)  
...
bs_func = function(i, idx_train=idx_train, model_params) { #epimedtools to see evolution of running 
