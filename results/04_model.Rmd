...
idx_train %in% rownames(df)
sum(!idx_train %in% rownames(df))
sum(!rownames(df) %in% idx_train)

x = as.matrix(df[idx_train,idx_cpg])
y = df[idx_train, y_key] # service. Design Oatterns, Gamma et al.

alpha=1

modelcv = glmnet::cv.glmnet       (x=x, y=y, alpha=alpha, type.measure="mse", standardize=TRUE)
modelcva = glmnetUtils::cva.glmnet(x=x, y=y, alpha=alpha, type.measure="mse", standardize=TRUE)
# modelcva_orig = mcvaglmnet(idx_train=idx_train, y_key=y_key)

dev.off()
m1 = modelcv
m2 = modelcva$modlist[[1]]
m3 = modelcva_orig$modlist[[11]]
layout(matrix(1:3,1),respect = TRUE)
plot(log(m1$lambda), sqrt(m1$cvm))
abline(v=log(m1$lambda.min))
plot(log(m2$lambda), sqrt(m2$cvm))
abline(v=log(m2$lambda.min))
plot(log(m3$lambda), sqrt(m3$cvm))
abline(v=log(m3$lambda.min))

layout(matrix(1:3,1),respect = TRUE)
plot(log(m1$lambda), m1$nzero)
abline(v=log(m1$lambda.min))
plot(log(m2$lambda), m2$nzero)
abline(v=log(m2$lambda.min))
plot(log(m3$lambda), m3$nzero)
abline(v=log(m3$lambda.min))
...
plot( log(modelcv$lambda), log(modelcva$modlist[[1]]$lambda))
plot(sqrt(modelcv$cvm) ,  sqrt(modelcva$modlist[[1]]$cvm))

modelcva$modlist[[1]]$lambda

modelcv$lambda
modelcva$modlist[[11]]$lambda
...
m_glmnet = mmodel_factory_glmnet(idx_train=idx_train, y_key=y_key, alpha=alpha, lambda=lambda) 
# prediction 
predTr = Xtrain[,rownames(m_glmnet$coeff)] %*% as.matrix(m_glmnet$coeff$beta) + m_glmnet$Intercept
rmseTr = sqrt(mean((Ytrain - predTr)^2))
predTe = Xtest[,rownames(m_glmnet$coeff)] %*% as.matrix(m_glmnet$coeff$beta) + m_glmnet$Intercept
rmseTe = sqrt(mean((Ytest - predTe)^2))
```

```{r plot prediction for best model, echo=FALSE}
layout(matrix(1:2,1), respect=TRUE)
plot(Ytrain, predTr, xlab="Chronological Age", ylab="Predicted Age", main=paste0(m_glmnet$name, " train rmse: ", signif(rmseTr, 3)))
plot(Ytest , predTe, xlab="Chronological Age", ylab="Predicted Age", main=paste0(m_glmnet$name, " test  rmse: ", signif(rmseTe, 3))) 
```
...
To create the new train set, we sample 2/3 of sample on original train set without replace and 1/3 with replace. That create a new train set used to make our model. We repete this method n_boot times and, at the end, we select for our final model only the markers shows on more than a certain pourcentage on bootstraps models. That create a final model more robusts than if we just make one model on our original train set, so this method increases robustness of results.
...
```{r bootstrap distribution, echo=FALSE}
layout(matrix(1:2,1),respect = TRUE)
...
stats = epimedtools::monitored_apply(mod=1, t(t(sort(unique(tmp_tab[tmp_tab>=n_boot/2])))), 1, function(occ) {
  # print(occ)
...
# layout(matrix(1:2,1),respect = TRUE)
# foo = boxplot(train_err~occurence, data=stats, border=1, las=2, ylab="RMSE", main="Cross Validation", xlab="probes occurence")
# bar = boxplot( test_err~occurence, data=stats, border=2, las=2, ylab="RMSE", main="Cross Validation", add=TRUE, xlab="probes occurence")
# idx = 2
# for(lty in 3:1) {
...
#   # x = (idx:ncol(foo$stats))
#   # y = foo$stats[3,idx:ncol(foo$stats)]
#   # points(x, y, pch=16)
#   # m = lm(y ~ x)
#   # abline(m,lty=lty)
# }
...
#
# boxplot(train_err~nb_probes, data=stats, border=1, las=2, ylab="RMSE", main="Cross Validation", xlab="nb_probes")
# boxplot( test_err~nb_probes, data=stats, border=2, las=2, ylab="RMSE", main="Cross Validation", add=TRUE, xlab="nb_probes")
# legend("bottomleft", col=1:2, c("train", "test"), pch=1)
# abline(v=ncol(foo$stats)-idx, col=2, lty=1)
...
models_bs = lapply(1:length(boot_probes), function(i){ # Compute coefficients and mean in the train for each probes of the model for each alpha
  ...
  idx = boot_probes[[i]]
  idx = idx[!is.na(m$coefficients[idx])]
  beta = m$coefficients[idx]
...
models = list(m_glmnet,models_bs[[1]])
...
for (it in covariates) {
  ...
  for (i in 1:length(models)) {
    ...
    colnames(AMAR_Te) = c("AMAR",it)
  	...
  	plot(predTr, Ytrain, xlab="Predicted Age", ylab="Chronological Age", main = paste0(m$name, " train rmse: ", signif(rmseTr, 3)),col = tmp_col[AMAR_Te[,it]])
    plot(predTe, Ytest , xlab="Predicted Age", ylab="Chronological Age", main = paste0(m$name, " test  rmse: ", signif(rmseTe, 3)),col = tmp_col[AMAR_Te[,it]])
    ...
    plot(0,0, xlim = c(min(unlist(lapply(density, function(d){min(d$x)}))),max(unlist(lapply(density, function(d){max(d$x)})))), ylim = c(min(unlist(lapply(density, function(d){min(d$y)}))),max(unlist(lapply(density, function(d){max(d$y)})))), main = paste0("density for ",m$name))
		...
		legend(x="topright", legend=factors, col=tmp_col, lty = 1,cex = 0.75, title="factor")
  }
}
