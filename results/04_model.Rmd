...
best_model_cvaglmnet_probes = m$coeff$probes
sub_df = df[idx_train, c(best_model_cvaglmnet_probes,y_key)]
best_model_cvaglmnet = build_model_from_probes(best_model_cvaglmnet_probes, sub_df, y_key, name="best_model_cvaglmnet")
...
stats = epimedtools::monitored_apply(mod=1, t(t(sort(unique(pb_occ_tab[pb_occ_tab>=n_boot/2])))), 1, function(occ) {
  # print(occ)
...
best_model_bs = build_model_from_probes(best_model_bs_probes, sub_df, y_key, name="best_model_bs")
```
...
hannum_model = build_model_from_probes(hannum_probes, sub_df, y_key, name="hannum_model")
...
horvath_model = build_model_from_probes(horvath_probes, sub_df, y_key, name="horvath_model")

models = list(best_model_cvaglmnet, best_model_bs, hannum_model, horvath_model, horvath_model)


for (covariate in covariates) {
  factors = unique(df[,covariate])
  ...
  for (m in models) {
    ...
    plot(Ytrain, predTr, xlab="Chronological Age", ylab="Predicted Age", main = paste0(m$name, " train rmse: ", signif(rmseTr, 3)), sub=paste0(nrow(m$coeff), " probes"))
		legend(x="topleft", legend=factors, col=tmp_col, lty = 1, title="factor")
    abline(a=0, b=1)
    plot(Ytest , predTe, xlab="Chronological Age", ylab="Predicted Age", main = paste0(m$name, " test  rmse: ", signif(rmseTe, 3)), sub=paste0(nrow(m$coeff), " probes"))
    abline(a=0, b=1)
    ...
    density = lapply(factors, function(factor){
      if(sum(AMAR_Te[,covariate] == factor) > 1) {
        return(density(AMAR_Te[AMAR_Te[,covariate] == factor, "AMAR"]))
      } # Problem density
    })
