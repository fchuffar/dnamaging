...
best_model_bs = build_model_from_probes(best_model_bs_probes, sub_df, y_key, name="Bootstraped model")
```
...
plot_model_eval = function(m, df, covariate) {
  ...
  abline(a=0, b=1)
  # AMAR Bias over Age
  # n=1000; x=sort(floor(runif(n)*100)); y = x+5*rnorm(n); plot(x, y); plot(density(y/x)); lines(density(y[1:500]/x[1:500]), col=2); lines(density(y[501:1000]/x[501:1000]), col=4)
...
for (covariate in covariates) {
  ...
  for (m in models) {
    plot_model_eval(m, df, covariate)
  }
}
