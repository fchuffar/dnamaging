...
for (cov in covariates) {
  ...
  tmp_col = RColorBrewer::brewer.pal(n=dim(tmp_tab), name = "Set1")
  ...
  beanplot::beanplot(formula(paste0(y_key,"~",cov)), data=df, method="jitter", log="",ylab=y_key)
  # text(x = 1.5, y = 110, labels = paste0("Wilcoxon p-val =", WTG$p.value))

  # boxplot(formula(paste0(y_key,"~",cov)), data=df, col=adjustcolor(tmp_col, alpha.f=.5), ylab=y_key)
  # # text(x = 1.5, y = 90, labels = paste0("Wilcoxon p-val =", WTG$p.value))

  beeswarm::beeswarm(formula(paste0(y_key,"~",cov)), data=df, ylab=y_key, col=tmp_col)  
}
```
