...
for (i in 1:5) {
    j = i+1
    plot(  pca$x   [,i], pca$x   [,j], col=adjustcolor(
                                           as.numeric(df[rownames(pca$x),]$pred_sf), alpha.f=0.3), pch=16, xlab=paste0("PC", i, "(", signif(p[i], 3), "%)"), ylab=paste0("PC", j, "(", signif(p[j], 3), "%)"))  
...
umap_test = predict(umap, data=x_test)
umap_grim = predict(umap, data=x_grim)

# rownames(umap_trai$layout)
layout(matrix(1:2,1), respect=TRUE)
plot(  umap_trai$layout            , col=adjustcolor(
                                         as.numeric(df[rownames(umap_trai$layout),]$pred_sf), alpha.f=0.1), pch=16 )
