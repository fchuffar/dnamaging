...
```{r linear model}
test_genes = intersect(gene_clusters[[3]], rownames(data))
length(test_genes)
...
plot(data_long$Age, data_long$Expression, col = as.numeric(as.factor(data_long$Gene)))

m = lm(Expression ~ Age, data = data_long)
abline(m)

m2 = lm(Expression ~ Age * Gene, data = data_long)
```
