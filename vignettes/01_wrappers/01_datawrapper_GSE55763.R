...
if (!file.exists(s_betatabfile_filename)) {
  ...
  if (!file.exists(paste0(gse, "_normalized_betas.txt.gz"))) {
    cmd = "wget"
    args = paste0("https://ftp.ncbi.nlm.nih.gov/geo/series/", substr(gse, 1, nchar(gse)-3), "nnn/", gse, "/suppl/", gse, "_normalized_betas.txt.gz")
  ...
  normalized_betas_matrix_file = paste0("~/projects/datashare/", gse, "/", gse, "_normalized_betas.txt.gz")
  sep="\t"
  ...
  s$exp_grp$key = paste0("X", substr(s$exp_grp$title, 19, 100000))
  s$exp_grp$key
