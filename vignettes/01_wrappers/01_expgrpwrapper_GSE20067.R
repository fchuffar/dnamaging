# 1. age
s$exp_grp$"ageatdiagnosis:ch1"
s$exp_grp$age = as.numeric(as.character(s$exp_grp$"ageatdiagnosis:ch1"))
sort(s$exp_grp$age)
...
# 5. disease
s$exp_grp$disease = as.factor(s$exp_grp$"case/control:ch1")
table(s$exp_grp$disease, useNA="always")
levels(s$exp_grp$disease) = c("T1D renal disease", "T1D control")
table(s$exp_grp$disease, useNA="always")
