...
sub_cvrecaps_nbp = sub_cvrecaps[sub_cvrecaps$nbprobes<=1*length(idx_train),]
best_model_cvaglmnet_params = sub_cvrecaps_nbp[sub_cvrecaps_nbp$cvm==min(sub_cvrecaps_nbp$cvm),][1,]
best_model_cvaglmnet = mmodel_factory_glmnet(idx_train=idx_train, y_key=y_key, alpha=best_model_cvaglmnet_params$alpha, lambda=best_model_cvaglmnet_params$lambda, gse=gse, idx_cpg=idx_cpg) 
...
best_model_bs_probes = names(pb_occ_tab[pb_occ_tab>=x_optim])
l_probes = length(best_model_bs_probes)

x_tmp = x
y_tmp = y
while(l_probes < 2) {
  x_tmp = x_tmp[-which(x_tmp == x_optim)] 
  y_tmp = y_tmp[-which(y_tmp == min(y_tmp))]
  x_optim = x_tmp[which(y_tmp==min(y_tmp))]
  best_model_bs_probes = names(pb_occ_tab[pb_occ_tab>=x_optim])
  l_probes = length(best_model_bs_probes)
}
...
layout(matrix(1:2,1),respect = TRUE)
plot(x, y, col=2, main="Cross validation", xlab="minimal occurence", ylab="RMSE", ylim=c(0, max(stats$test_err)), pch=3)
abline(v=x_optim, col=2)
points(x, ytr, col=1, pch=3)
abline(h=tmp_rmse, col=2, lty=3)
legend("topleft", paste0("RMSE=", signif(tmp_rmse,3)), col=2, lty=3, cex=.5)
...
points(stats$occurence, stats$test_err, col=adjustcolor(2, alpha.f=.1))
points(stats$occurence, stats$train_err, col=adjustcolor(1, alpha.f=.1))

par(mar=c(5.1, 4.1, 4.1, 2.1))
legend("bottomright", c("Nested train +/- 1se", "Validation +/- 1se", paste0(x_optim, " occ. over ", n_boot, " bs")), 
  pch=c(3,3,NA), 
  col=c(1, 2, 2),
  lty=c(NA,NA,1), 
  cex=.7
...
foo = cumsum(rev(table(pb_occ_tab)))
plot(as.numeric(names(foo)), foo, las=2, 
  xlab=paste0("cumulated probes occurence (for ", n_boot, " bootstraps)"), 
  ylab="nb. probes",
  main=paste0("Cumulated probes occurence"), 
  xlim=c(500,1), lty=1
...
abline(v=x_optim, col=2)
abline(h=length(best_model_bs_probes), col=2)
legend("topleft", paste0(length(best_model_bs_probes), " probes viewed", x_optim, " times"), col=2, lty=1, cex=.7)
