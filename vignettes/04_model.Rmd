...
if (!exists("typeofmodel")) typeofmodel = "bs"
...
print("Loading data")
if (run == 0) {
  df = mreadRDS(paste0("df_r", run, "_ewas", nbewasprobes, "_nn", dist_nn, "_", gse, ".rds"))
}else { df = mreadRDS(paste0("df_r", run, "_", typeofmodel, "_ewas", nbewasprobes, "_nn", dist_nn, "_", gse, ".rds")) }
...
saveRDS(list(best_model_cvaglmnet, best_model_bs), paste0("models_r", run,"_",typeofmodel, "_ewas", nbewasprobes, "_nn", dist_nn, "_", gse, ".rds"))
```
...
for (covariate in covariates) {
  ...
  info[[length(info)+1]] = info_cov 
}
names(info) = covariates
...
if(typeofmodel == "bs"){
  ...
  dim(df)
  saveRDS(df,paste0("df_r", run+1, "_bs", "_ewas", nbewasprobes, "_nn", dist_nn, "_", gse, ".rds"))
}

if(typeofmodel == "glm"){
  dim(df)
  df = df [,!(colnames(df)%in%rownames(best_model_cvaglmnet$coeff))]
  dim(df)
  saveRDS(df,paste0("df_r", run+1, "_glm", "_ewas", nbewasprobes, "_nn", dist_nn, "_", gse, ".rds"))
}
...
```{r}
stop_time = Sys.time()
info$exec_time = round(difftime(stop_time, start_time, units="mins"),1)

saveRDS(info, paste0("info_model_r", run,"_", typeofmodel, "_ewas", nbewasprobes, "_nn", dist_nn, "_", gse, ".rds"))
```
