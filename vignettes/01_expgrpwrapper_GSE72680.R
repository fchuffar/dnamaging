...
# 7. ethnicity
s$exp_grp$ethnicity = as.character(s$exp_grp$"race/ethnicity:ch1")
table(s$exp_grp$ethnicity, useNA="always")
s$exp_grp[s$exp_grp$ethnicity%in%c("Mixed", "Other") ,]$ethnicity = NA
table(s$exp_grp$ethnicity, useNA="always")
s$exp_grp$ethnicity = as.factor(s$exp_grp$ethnicity)
table(s$exp_grp$ethnicity, useNA="always")
