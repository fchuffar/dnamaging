...
newas = "1000000"
neighb = 1000
tmp = list.files(pattern="*_CustGSE147740rr_modelcalllm_meth~age_ewas1000000_nn1000.rds")
runmax = max(as.numeric(substr(do.call(rbind, strsplit(tmp[grep("info_model", tmp)], "_"))[,3], 2, 1000)))
...
all(rownames(pfepichg38) %in% rownames(ewas))
all(rownames(ewas) %in% rownames(pfepichg38))
ewas = ewas[rownames(ewas) %in% rownames(pfepichg38),]
all(rownames(ewas) %in% rownames(pfepichg38))
...
signatures[["hannum"   ]]  = rownames(litterature_models$hannum_model_mc$coeff)
signatures[["gao"     ]]  = openxlsx::read.xlsx("../misc/13059_2023_2953_MOESM2_ESM.xlsx", sheet=16)[-(1:2),2] # https://genomebiology.biomedcentral.com/articles/10.1186/s13059-023-02953-6#Sec27
signatures[["horvath"  ]]  = rownames(litterature_models$horvath_model_mc$coeff)
signatures[["phenoage" ]]  = read.table("aging-v10i4-101414-supplementary-material-SD2.csv", sep=",", header=TRUE)$CpG[-1]
signatures[["predimeth"]]  = bed_predimeth_hg19[,4]
signatures[["morandini"]]  = openxlsx::read.xlsx("../misc/43587_2024_757_MOESM5_ESM.xlsx")[-1,1]
# signatures[["zeng"     ]]  = openxlsx::read.xlsx("../misc/41467_2019_9301_MOESM4_ESM.xlsx")[,1] # https://pmc.ncbi.nlm.nih.gov/articles/PMC6437195/
predimeth_it = bed_predimeth_hg19[,5]
names(predimeth_it) = rownames(bed_predimeth_hg19)

ewas$predimeth = 0
ewas[intersect(signatures[["predimeth"]], rownames(ewas)),]$predimeth = 1
...
for (k in names(signatures)) {
  ewas[[k]] = 0
  ewas[intersect(signatures[[k]], rownames(ewas)),k] = 1
}

# probes_signature = rownames(ewas)[apply(ewas[, names(signatures)], 1, sum) > 0] 
probes_signature = rownames(ewas)[ewas$predimeth==1] 
...
# ewas$morandini = 0
# ewas[intersect(probes_morandini, rownames(ewas)),]$morandini = 1
# ewas$zeng = 0
# ewas[intersect(probes_zeng, rownames(ewas)),]$zeng = 1
...
```{r child = '06_postprocpredimethaging.Rmd', eval=TRUE}
```
...
```{r child = '05_postprocewas.Rmd', eval=FALSE}
```
...
```{r child = '06_postprocpredimethbiology.Rmd', eval=TRUE}
```
...
```{r child = '05_postprocpredimethstats.Rmd', eval=FALSE}
```
...
```{r child = '05_postprocgsea.Rmd', eval=FALSE}
```

```{r child = '05_postprocpancancer.Rmd', eval=FALSE}
```



```{r child = '06_postprocpredimethcancer.Rmd', eval=TRUE}
```
