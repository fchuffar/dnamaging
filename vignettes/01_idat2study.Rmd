...
epimedtools::download_gse_raw_tar(gse, datashare_dir="datashare")
```
...
data.dir = paste0("datashare/", gse, "/raw")
...
detect_infinium_platform = function (idat.fnames) { # inspired by RnBeads:::rnb.detect.infinium.platform
        ...
        if (inf450kEPIC.idats.present) {
            file.sizes <- as.numeric(na.omit(file.info(idat.fnames)[, 
                "size"]))
...
setwd(wd_orig)
idat_files = list.files(data.dir, "*_Red.idat")
idat_files = idat_files[substr(idat_files, 1, 3)!="GSM"]
setwd(data.dir)
bar = sapply(idat_files, detect_infinium_platform)
tab = table(bar)
if (length(tab) > 1) {
  pf_idx = names(rev(sort(tab)))[1]
}
idat_files = idat_files[bar==pf_idx]
sentrix_idx = unique(do.call(rbind, strsplit(idat_files, "_"))[,1])
setwd(wd_orig)
...
dim(exp_grp)
exp_grp = exp_grp[exp_grp$Sentrix_ID%in%sentrix_idx,]
dim(exp_grp)
                                ...
                                if (length(data.source) == 1L) {
                                    ...
                                    if (!is.data.frame(data.source[[2]])) {
                                      ...
                                      { 
                                        ...
                                        platform <- RnBeads:::detect_infinium_platform(idat.fnames)
                                        if (verbose) {
