...
epimedtools::download_gse_raw_tar(gse, datashare_dir=datashare_dir)
```
...
if (pf_idx=="probesEPIC") {
  nb_sample_max = 2477 
} else if (pf_idx=="probes450") {
  nb_sample_max = 5000  
} else {
  nb_sample_max = 80000    
}
...
nb_sentrix = length(levels(exp_grp$Sentrix_ID))
set.seed(1)
batch_sentrix_dict = sample(1:nb_batch, nb_sentrix, replace=TRUE)
names(batch_sentrix_dict) = levels(exp_grp$Sentrix_ID)
...
exp_grp = exp_grp[intersect(rownames(exp_grp), colnames(data)),]
head(exp_grp)
dim(exp_grp)
