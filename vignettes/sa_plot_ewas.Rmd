...
ns = n = 100
p = seed = 1
results_files = list.files(pattern=paste0("results_", gse, "_nbewasprobes*"))
...
stats = lapply(results_files, function(results_file) { 
        ...
        if (file.exists(results_file)) {
          ...
          results$nbewasprobes= as.numeric(substr(rev(strsplit(results_file, "_")[[1]])[2], 13, 100))
          results$gse     = gse  
          return(results)
        } else {
          NULL
...
stats = stats[,c("nbewasprobes", "seed", keys)]
for (ylab in keys) {   
  ...
  formula = as.formula(paste0(ylab, "~nbewasprobes"))
  foo = boxplot(formula, stats, las=2, main=gse, ylab=ylab, ylim=ylim)
  
  # legend(x="bottomleft", legend=ns, fill=1:length(ns), title="n")
}

layout(matrix(1:2,1), respect=TRUE)
plot(stats$nbewasprobes, stats$rmsemod1_test, col=stats$seed)
plot(stats$nbewasprobes, stats$rmsemod2_test, col=stats$seed)
```
