...
premimeth_probes = read.table("predimeth_probes_GSE147740_50.bed")[,4]
premimeth_probes_per_bed_files = lapply(names(probes_per_bed_files), function(bed_file) {  
  tmp_pbs = premimeth_probes[premimeth_probes %in% probes_per_bed_files[[bed_file]]]
...
grp_files = sapply(names(metaclusters), function(k) {
  tmp_pbs = premimeth_probes_per_bed_files[[paste0(k, ".bed")]]
...
chi = data.frame(
  ...
  exp = round(sapply(probes_per_bed_files, length) / length(probes_per_bed_files$bon) * length(premimeth_probes_per_bed_files$bon))
)
