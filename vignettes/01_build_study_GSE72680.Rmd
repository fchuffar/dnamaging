---
title: "Build Illumina methylation study for GSE72680"
...
s1 = read.table("~/Téléchargements/GSE72680_beta_values.txt",header=TRUE,sep="\t")
library("dplyr")
s1 = s1 %>% select(-contains("Detection")) #Removing detection pval columns 
colname = s1[,1]
rownames(s1) = colname
s1 = s1[,-1]
dim(s1)
colnames(s1) = colnames(foo)
