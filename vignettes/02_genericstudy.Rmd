...
# # A. remove probes on chrX and chrY
# head(s$platform[,1:6])
# table(s$platform[,1])
...
# C. if epicV2, convert to epicV1
if (s$platform_name%in%c("GPL33022")) {
  sepicv2_filename = paste0("./datashare/", gse, "/study_epicv2_", gse, ".rds")
  print(paste0("Writing ", sepicv2_filename, "..."))
  s$save(sepicv2_filename)
  dim(s$data)
  dim(s$exp_grp)
  dim(s$platform)
  s$platform = s$platform[s$platform$EPICv1_Loci!="",]
  dim(s$platform)
  s$platform = s$platform[!duplicated(s$platform$EPICv1_Loci),]
  dim(s$platform)
  s$data = s$data[rownames(s$platform),]
  rownames(s$platform) = s$platform$EPICv1_Loci
  rownames(s$data) = rownames(s$platform) 
  dim(s$data)
  # stop("EFN")
}
...
# D. cell composition
if (s$platform_name %in% c("GPL21145", "GPL23976", "GPL13534", "GPL33022")) {
  library(EpiDISH)
  betaData.m = s$data
  dim(s$data)
