...
if (!file.exists(s_betatabfile_filename)) {
  ...
  postfix = "_EUGEI_processed_signals.csv.gz"
  ...
  s$exp_grp$key = paste0("X", substr(as.character(s$exp_grp$title), 1, 19))
  s$exp_grp$key
  sum(!colnames(tmp_data) %in% s$exp_grp$key)
  tmp_data = tmp_data[,colnames(tmp_data) %in% s$exp_grp$key]
  sum(!colnames(tmp_data) %in% s$exp_grp$key)
